...
- Name: features\auth-register.spec.ts >> Feature: User Registration >> accepts registration with French locale
- Location: tests\features\auth-register.spec.ts:100:7
...
> 115 |     expect(res.status()).toBe(200)
  ...
  117 |     expect(body.success).toBe(true)
  118 |   })
  119 |
  120 |   test('rejects an invalid locale', async ({ request, baseURL }) => {
  121 |     const res = await request.post(`${baseURL}${ENDPOINT}`, {
  122 |       data: {
  123 |         email: `locale+${Date.now()}@example.com`,
  ...
  132 |     expect(body.error).toBe('VALIDATION_ERROR')
  133 |   })
  134 |
  135 |   test('rejects request with missing required fields', async ({
  136 |     request,
  137 |     baseURL,
  138 |   }) => {
  139 |     const res = await request.post(`${baseURL}${ENDPOINT}`, {
  140 |       data: {},
  141 |     })
  142 |
  143 |     expect(res.status()).toBe(400)
  144 |     const body = await res.json()
  145 |     expect(body.error).toBe('VALIDATION_ERROR')
  146 |   })
  147 | })
  148 |
```
